...
import { AlarmClockCheck, Captions, Home } from 'lucide-react'
        ...
        <div className='w-full h-full bg-slate-900 relative'>
            ...
            <Link href="/staff/projects">
                <Tooltip title="Projects" placement='right'><motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                    ...
                    <div className="flex gap-1"><Captions size={18} /> Your Projects</div> {projectsAnalytics?.unopenedProjects?.length > 0 && <Badge>{projectsAnalytics?.unopenedProjects?.length}</Badge>}
                </motion.button></Tooltip>
            </Link>
        </div>
    )
