...
export async function POST(req: NextRequest){
    try {
        ...
        const updatedStaff = await Users.findByIdAndUpdate(existing?.RegionHead, { Role: 'staff' })
        ...
        const existNotif = await sendNotification("Your Position Changed", `Your Current Postion as "Region Head" of ${existing?.RegionName} have been changed. you may not have some privillages from now on.`, session?.user?.id, existing?.RegionHead);
